...
import { insertCartMock } from '../../cart-product/__mocks__/insert-cart.mock';
...
describe('CartService', () => {
  ...
  it('should return error in findOne undefined', async () => {
    ...
    await expect(service.clearCart(userEntityMock.id)).rejects.toThrowError(
      NotFoundException,
    );
  });
  ...
  it('should return notFoundException in not found cart', async () => {
    ...
    await expect(service.findCartByUserId(userEntityMock.id)).rejects.toThrowError(
      NotFoundException,
    );
  });
