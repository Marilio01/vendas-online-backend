import { JwtService } from '@nestjs/jwt';
 ...
 describe('AuthService', () => {
   ...
   it('should throw NotFoundException if email does not exist', async () => {
    jest.spyOn(userService, 'findUserByEmail').mockRejectedValue(new NotFoundException()); // Rejeita com NotFoundException

    await expect(service.login(loginUserMock)).rejects.toThrowError(NotFoundException);
});
